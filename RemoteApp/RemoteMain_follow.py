...
import SendData_Car
# import SendData_PC_client

import ReceiveData_Car
import ReceiveData_PC_client
...
#  some queue but , receive from Preceding
# Acceleration_CAR_Pre_QUEUE = queue.Queue(1)
Dist_Vel_Accel_CAR_Pre_QUEUE = queue.Queue(1)
...
# video_source = 'http://192.168.91.187:4747/video'  # phone 
...
class RemoteMain(object):
    ...
    def setup_ui(self, main_window):
        ...
        self.__send_client_CAR = SendData_Car.SendData_Car(self.__host , 3344)
        self.__send_client_CAR.log_signal.connect(self.update_log)

        self.__recive_data_client_Phone = ReceiveData_Phone.ReceiveData_Phone(self.__host,7778)
        self.__recive_data_client_Phone.log_signal.connect(self.update_log)

        self.__recive_data_client_CAR = ReceiveData_Car.ReceiveData_Car(self.__host , 4455)
        self.__recive_data_client_CAR.log_signal.connect(self.update_log)

        self.__recive_data_from_server_PC= ReceiveData_PC_client.ReceiveData_PC_client(self.__host)
        self.__recive_data_from_server_PC.log_signal.connect(self.update_log)
        ...
        self.__recorder.daemon = (True)
        self.__recorder.start()

        """ That config for Following CAR (1 send , 3 receive) """


        # # thread send commands to car for control the motor and servo 
        ...
        # ---------- Receive self car data  Acceleration , velocity from car preceding --------

        # # ---- velocity distance from car sensor
        self.__recive_data_client_CAR_thread = \
            threading.Thread(target=ReceiveData_Car.ReceiveData_Car.update_car_distance_vel_follow, \
            args=(self.__recive_data_client_CAR, Distance_Velocity_CAR_QUEUE))
        self.__recive_data_client_CAR_thread.daemon = (True)
        self.__recive_data_client_CAR_thread.start()

        # # # ---- acceleration from phone sensor (act like car sensor)
        self.__recive_data_client_Phone_thread = \
            threading.Thread(target=ReceiveData_Phone.ReceiveData_Phone.update_car_acceleration_follow, \
            args=(self.__recive_data_client_Phone,Acceleration_CAR_QUEUE))
        ...
        # self.__recive_data_from_server_PC_thread = \
        #     threading.Thread(target=ReceiveData_PC_client.ReceiveData_PC_client.update_car_pre_accel_vel_distance, \
        #     args=(self.__recive_data_from_server_PC, Dist_Vel_Accel_CAR_Pre_QUEUE ))
        # self.__recive_data_from_server_PC_thread.daemon = (True)
        # self.__recive_data_from_server_PC_thread.start()
        ...
        #     threading.Thread(target=Analyser.Analyser.analyse_follow, \
        #     args=(self.__analyser, FRAME_QUEUE, ANALYSED_FRAME_QUEUE, \
        #     COMMANDS_QUEUE, CAR_STATES_QUEUE, Acceleration_CAR_QUEUE , Distance_Velocity_CAR_QUEUE, Dist_Vel_Accel_CAR_Pre_QUEUE))
        ...
        # new 
        self.__cacc_thread = \
            threading.Thread(target=Analyser.Analyser.CO_apdativeCC, \
            args=(self.__analyser, COMMANDS_QUEUE,Acceleration_CAR_QUEUE , Distance_Velocity_CAR_QUEUE, Dist_Vel_Accel_CAR_Pre_QUEUE))
        self.__cacc_thread.daemon = True
        self.__cacc_thread.start()
        # self.__cacc_thread.is_analysing = self.__acc_activated
    ...
    def retranslate_ui(self, main_window):
        ...
        main_window.setWindowTitle(\
            _translate("main_window", "Adaptive Cruise Control - Following", None))
    ...
    def key_press_dynamic(self, event):
        ...
        if key == QtCore.Qt.Key_Escape:
            ...
            data = CMD_STEER + ":" + str(self.utils.map_value_Steer_ESP1(self.steer)) + "\n"

            COMMANDS_QUEUE.put(data, True, None)
            # COMMANDS_QUEUE.put(CMD_GO_LEFT, True, None)

            # self.__send_client_CAR.send_direct_commands(f'STEER:{self.utils.map_value_Steer_ESP1(self.steer)}\n')
            ...
            self.__send_client_CAR.send_direct_commands(f'STEER:{self.utils.map_value_Steer_ESP1(0)}\n')
        ...
        # elif key == QtCore.Qt.Key_R:
        #     self.__send_client_CAR.send_direct_commands(f'BACKWARD:{self.utils.map_value_Steer_ESP1(self.steer)}\n')
